...
const Folder = ({ name, id, optimistic, count }: Props) => {
  ...
  const { mutate, isPending } = useMutationData(['rename-folder'], (data: { name: string }) => renameFolder(id, data.name), 'workspace-folders', Renamed)
        ...
        <div className="flex flex-col gap[1px]">
          {
            onRename ? (
              ...
              autoFocus ref={inputRef} className='border-none underline  text-base w-full outline-none text-neutral-300 bg-transparent p-0' />
