    ...
    return (
        <div className='flex flex-col gap-4'>
            ...
            <section className={cn(status !== 200 && "justify-center", 'flex items-center gap-4 overflow-x-auto w-full')}>
                {
                    status !== 200  ? <p>No folders in workspace</p> : <>{latestVariables && latestVariables.status === "pending" && <Folder optimistic name={latestVariables.variables.name} count={10} id={latestVariables.variables.id} />}</> 
         }
         {
          folders &&  folders?.folders?.map((folder) => (
                <Folder key={folder.id} name={folder.name} count={folder._count.videos} id={folder.id} />
            ))
         }
            </section>
        </div>
