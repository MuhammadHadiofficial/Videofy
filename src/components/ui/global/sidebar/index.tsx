...
const Sidebar = ({ activeWorkspaceId }: Props) => {
  ...
  const { data: count } =
    notifications ?? ({ data: undefined } as unknown as NotificationProps);
  ...
  const Sidebar = (
    <div className="bg-[#111111] flex-none relative p-4 h-full w-[250px] flex flex-col gap-4 items-center overflow-hidden">
      ...
      <>{currentWorkspace?.type + currentWorkspace?.subscription?.plan}</>
              ...
              <div className="w-full h-full flex justify-center items-center">
                <p className="text-[#9D9D9D] font-medium text-sm">
                  {workspace.subscription?.plan === "FREE"
                    ? "Upgrade to the pro plan to create more workspaces."
                    : "No workspaces."}
                </p>
              </div>
            )}
